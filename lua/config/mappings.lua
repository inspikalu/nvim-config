...
-- Toggle focus between NvimTree and current buffer
map('n', '<leader>te', function()
  local view = require('nvim-tree.view')
  if view.is_visible() then
    local current_win = vim.api.nvim_get_current_win()
    local tree_win = view.get_winnr()
    -- If focused on tree, switch to last window; else focus tree
    if current_win == tree_win then
      vim.cmd('wincmd p')  -- Switch to previous window
    else
      view.focus()  -- Focus the tree
    end
  else
    vim.cmd('NvimTreeToggle')  -- Open tree if closed
  end
end)
